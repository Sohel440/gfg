...
int solve(int i, int j, string &s , unordered_set<string> &st) {
    if (j >= s.size()) return 0;

    int count = 0;

    // Check if the substring from i to j is divisible by 4
    if (stoll(s.substr(i, j - i + 1)) % 4 == 0) {
       st.insert(s.substr(i,j-i+1));
        count = 1;
    }

    // Option 1: Do not extend the current substring (move `j` forward)
    int nontake = solve(i, j + 1, s , st);

    // Option 2: Start a new substring from `j + 1`
    int take = solve(j + 1, j + 1, s , st);

    // Return the sum of the current count and the results from both options
    return count + nontake + take;
}
