...
void kaj() {
    ...
    sort(sohel.rbegin(), sohel.rend());

    int mx = 0;
    for (int i = 0; i < min(n, (int)sohel.size()); ++i) {
        mx += sohel[i];
    }
...
// int max_profit(int n, int k, const vector<pair<int, int>> &btl) {
//     
...
//         int n, k;
//         cin >> n >> k;
//         vector<pair<int, int>> btl(k);

//         // Read each brand-cost pair
//         for (int j = 0; j < k; ++j) {
//             cin >> btl[j].first >> btl[j].second;
//         }

//         // Calculate max profit for this case
//         results.push_back(max_profit(n, k, btl));
//     }
