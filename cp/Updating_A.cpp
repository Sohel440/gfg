/*int N;
        ...
        for (int i = 0; i < N; i++) {
            total_sum += arr[i];
            prefix_sums.insert(total_sum);
        }
        int min_diff = LLONG_MAX;
        total_sum = 0;
        for (int i = 0; i < N - 1; i++) {
            total_sum += arr[i];
            auto it = prefix_sums.find(total_sum);
            prefix_sums.erase(it);
            int next_sum = *(prefix_sums.begin());
            if (i != 0) {
                next_sum -= total_sum;
                next_sum += arr[i];
            }
            min_diff = min(min_diff, next_sum);
        }
        total_sum += arr[N - 1];
        cout << total_sum - min_diff + abs(min_diff) << endl;*/
