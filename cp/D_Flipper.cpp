...
void kaj() {
   ...
   vi ans(rall(a));
    ...
    // this case only when the n is at last position 
   forn(i , 0 , n){
    auto q = a;
   ...
   for(int i = 0 ; i < x ; i++){              // 2 3  1 5 4
    auto q = a;
    std::reverse(q.begin()+i , q.begin() + x );                // let  i = 1 , x  = 3 ,  2 1 3 5 4
    std::rotate(q.begin() , q.begin()+i , q.end());            //                        1 3 5 4 2
    std::rotate(q.begin(), q.begin() + x - i , q.end()-i);     // x = 3, i = 1 , q.b+2                 5 4 1 3 2 ans 
    ans = max(ans , q);
   }
