...
bool ok(const vector<int>& perm, int K) {
...
void kaj() {
    ...
    vector<int> a(n);
    for (int i = 0; i < n; ++i) {
        a[i] = i + 1; // Initialize array as [1, 2, ..., n]
    }

    forn(i,2,k+1) {
        vector<int> temp(a.end() - (i - 1), a.end()); 
        forn(i,0,temp.size())cout << temp[i]<<" ";
        el;
        temp.insert(temp.end(), a.begin(), a.end() - (i - 1)); 
