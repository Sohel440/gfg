...
void kaj() {  
  ...
  for(int i = 0 ; i < s.size(); i++){
   int digit = s[i] - '0';  // Convert character to its integer value
        int a = 9 - digit;       // Calculate the transformed value
        char ch = '0' + a;       // Convert the transformed value back to a character
        s[i] = min(ch, s[i]); 
