...
int cc(const line& O, const line& A, const line& B) {
    return (A.x - O.x) * (B.y - O.y) - (A.y - O.y) * (B.x - O.x);
}
double durotto(const line& A, const line& B) {
    return sqrt((A.x - B.x) * (A.x - B.x) + (A.y - B.y) * (A.y - B.y));
}
vi(line) me;
void sohel440(vi(line)& l) {
    ...
    for (const auto& it : l) {
        while (me.size() >= 2 && cc(me[me.size() - 2], me.back(), it) <= 0) {
            me.pop_back();
        }
        me.push_back(it);
    }
...
void kaj(){
    ...
    sohel440(points);
    double sohel = 0.0;
    for (size_t i = 1; i < me.size(); ++i) {
        sohel += durotto(me[i - 1], me[i]);
    }

    
    cout << static_cast<int>(round(sohel));
