...
void chines(vector<int> &nums , vector<int> &rem){
    ...
    for(auto u : nums) pr = pr * u;

    for(int i = 0 ; i < nums.size();i++){
        int ppi = pr / nums[i];
        int r = rem[i];
        int inverse = inv(ppi , nums[i]-2 , nums[i])  ;
...
jeno_joyer_khida_thake 
{
   ...
   mobius_korlam();
   for(int i = 2 ; i < 30 ; i++) cout << (a[i]) <<" "  << (prime_count[i]) << " " << (mobius[i]) << endl;
   pf('\n');
//    for(int i = 2 ; i < 30 ; i++) pf(prime_count[i]) << " ";
//    pf('\n');
//    for(int i = 2 ; i < 30 ; i++) pf(prime_count[i]) << " ";
//    pf('\n');

   cout << prime_count[42] <<" "<<  a[42];
