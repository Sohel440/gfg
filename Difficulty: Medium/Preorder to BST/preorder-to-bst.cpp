...
class Solution {
  public:
   ...
   Node* solve(int &i, int arr[],int mn , int mx ,int size) {
          
        if(i== size || arr[i] < mn || arr[i]> mx)return NULL;
        
        Node * root = newNode(arr[i++]);
        root->left = solve(i, arr ,    mn ,        root->data, size);
        root-> right = solve(i , arr ,root->data , mx,         size);
    ...
    Node* Bst(int pre[], int n) {
         int i =0;
        Node*root= solve(i , pre , INT_MIN, INT_MAX, n);
        return root;
    }
};
